...
def convert_annotation(image_id):
    ...
    im_path  = ('rot_img/rot%s.jpg'%(image_id))
    im = Image.open(im_path)
    xml_path = open('rot_xml/rot%s.xml'%(image_id))
...
if __name__ == '__main__':
    path='D:/postgraduateworking/cvdasai/data/VOC2007/labels'
    files=os.listdir(path)

    for file in files:
        name1, name2 = os.path.splitext(file)
        convert_annotation('%s'%name1)
